...
# TODO - add a do you want smb flag and corresponding SMB nukinator
...
$logPath = "C:\Logs"
$transcriptPath = Join-Path $logPath "virtual-viagara.log"
...
foreach ($keyPath in $autorunKeys) {
    if (Test-Path $keyPath) {
        ...
        $key.GetValueNames() | ForEach-Object {
            Remove-ItemProperty -Path $keyPath -Name $_ -Force
            Write-Host "Removed registry autorun entry: $($_) from $keyPath"
        }
    }
}
...
foreach ($path in $startupPaths) {
    if (Test-Path $path) {
        Get-ChildItem $path -File | ForEach-Object {
            ...
            Write-Host "Deleted startup file: $($_.Name)"
        }
    }
}

# DNS: Turn off multicast name resolution
...
# DNS: Secure cache against pollution
Add-DnsServerDirectoryPartition -Name "ForestDnsZones" -ReplicationScope Forest -ErrorAction SilentlyContinue
Set-DnsServerSetting -SecureResponses $true

# DNS: Zone Transfers to only Name Servers
$zoneName = "wh.local"
Set-DnsServerPrimaryZone -Name $zoneName -ZoneTransferType "NameServers"

# DNS: Enable DNS Event Logging
Set-DnsServerDiagnostics -EventLogLevel 2

# MS Network Client: Digitally sign communications
secedit /export /cfg "$env:TEMP\secpol.cfg"
(Get-Content "$env:TEMP\secpol.cfg") -replace 'Microsoft network client: Digitally sign communications \(always\) = .*', 'Microsoft network client: Digitally sign communications (always) = 1' | Set-Content "$env:TEMP\secpol.cfg"
secedit /configure /db secedit.sdb /cfg "$env:TEMP\secpol.cfg" /overwrite

# Kerberos: Set encryption types
$encTypes = 0x18  # AES128_HMAC_SHA1 + AES256_HMAC_SHA1
New-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control\Lsa\Kerberos\Parameters" -Name "SupportedEncryptionTypes" -Value $encTypes -PropertyType DWord -Force

# LDAP: Require signing
...
# Print Service: Prevent users from installing printer drivers
secedit /export /cfg "$env:TEMP\secpol2.cfg"
(Get-Content "$env:TEMP\secpol2.cfg") -replace 'Devices: Prevent users from installing printer drivers = .*', 'Devices: Prevent users from installing printer drivers = 1' | Set-Content "$env:TEMP\secpol2.cfg"
secedit /configure /db secedit2.sdb /cfg "$env:TEMP\secpol2.cfg" /overwrite

# CA Service Auditing
auditpol /set /subcategory:"Certification Services" /success:enable /failure:enable
auditpol /set /subcategory:"Certification Services - Backup and Restore" /success:enable /failure:enable
auditpol /set /subcategory:"Certification Services - Configuration Changes" /success:enable /failure:enable

# Also enable Auditing for Issue and Manage Certificate Requests via registry
$regPath = "HKLM:\SYSTEM\CurrentControlSet\Services\CertSvc\Configuration"
$caNames = Get-ChildItem $regPath
foreach ($ca in $caNames) {
    Set-ItemProperty -Path "$regPath\$ca" -Name "AuditFilter" -Value 127 -Type DWord
}
